...
sudo dd if=/dev/zero of=$TARGET bs=512 count=${DISK_SECTOR_COUNT} >/dev/null
...
sudo mkfs.fat -n "NQOS" $TARGET_PARTITION >/dev/null
...
# copy files
echo "Copying files to ${TARGET_PARTITION} (mounted on /tmp/nqos)..."
sudo mkdir -p /tmp/nqos
sudo mount ${TARGET_PARTITION} /tmp/nqos
sudo cp ${BUILD_DIR}/kernel.bin /tmp/nqos
sudo cp test.txt /tmp/nqos
sudo mkdir /tmp/nqos/test
sudo sudo cp test.txt /tmp/nqos/test
sudo umount /tmp/nqos
